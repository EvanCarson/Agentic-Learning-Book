...
function in `TOOLS`. Unknown names fall back to a plain answer. Note that each tool unpacks its own arguments (`add` splits the comma-packed string) — argument parsing is the tool's job, not the dispatcher's.
